...
create_topic_model_report <- function(model, report_dir, cov_dataset, raw_texts, diff_cov=NULL, diff_cov.value1=NULL, diff_cov.value2=NULL, 
    ...
    if(!is.null(diff_cov)) {
    	...
    	prep <- estimateEffect(fmla, m, metadata=as.data.frame(d))
	    plot.estimateEffect(prep, diff_cov, model=m, 
	                        method="difference", cov.value1=diff_cov.value1, 
	                        cov.value2=diff_cov.value2, verbose.labels=FALSE, 
	                        main=sprintf("Difference in Mean Topic Prevalence, %s minus %s", diff_cov.value1, diff_cov.value2))
	    dev.off()
    }

    d[ , era := cut(year, breaks=c(1976, 1990, 1995, 2000, 2005, 2010, 2015), 
    ...
    ymax <- max(topic_prevalence_by_year[ , setdiff(names(topic_prevalence_by_year), "era"), with=FALSE])
    for(topic in 1:num_topics) {
        png(filename=file.path(report_dir, paste0("Time Trend, Topic ", topic, ".png")))
        plot(x=1:nrow(topic_prevalence_by_year), y=topic_prevalence_by_year[[paste0("Topic_", topic)]], 
             type="o", main=paste0("Topic_", topic), ylab="Average Topic Score", xlab="Era", ylim=c(0, ymax), 
             lwd=2, xaxt="n")
        axis(side=1, at=1:6, labels=str_wrap(levels(topic_prevalence_by_year$era), width=15))
        dev.off()
    }
    ...
    #cat("HIGHEST FREX WORDS BY TOPIC\n\n\n", file=summary_file)

    # Write most representative abstract files
    repr <- findThoughts(m, n=10)$index
    ...
    for(topic in 1:num_topics) {
        ...
        for(rank in 1:10) {
            ...
            ti <- str_replace(names(raw_texts)[idx], "pdf|\\.txt", "")
            ti <- paste0(rank, ".", ti)
            if(representative_docs) {
                cat(raw, file=file.path(most_repr_topic_dir, 
                                        paste0(ti, ".txt")))
                print_color_coded_pdf(raw, topic_frex, filename=file.path(most_repr_topic_dir, 
                                                                          paste0(ti, ".pdf")))
            } else {
                cat(paste0(rank, ". ", ti, "\n"), file=file.path(most_repr_topic_dir, "titles.txt"), 
                    append=TRUE)
            }
#             yr <- d$year[repr[[topic]][rank]]
#             cat(raw_texts[repr[[topic]][rank]], file=file.path(most_repr_topic_dir, 
#                                                                 paste0(rank, "_", yr, ".txt")))
        }
        rm(topic_frex)
    }
    rm(pm)
...
#         
#         # Write summary file
#         cat(paste0("Topic ", topic, "\n\n"), file=summary_file, append=TRUE)
#         for(word in 1:15) {
#             cat(paste0(word, ". ", tmp$frex[topic, word], "\n"), file=summary_file, append=TRUE)
#         }
#         cat("\n\n", file=summary_file, append=TRUE)
#         
#         # Write detailed topic file
#         topic_file <- file.path(report_dir, paste0("Topic ", topic, ".txt"))
#         longest_frex_word <- max(sapply(tmp$frex[topic,], nchar)) + 1
#         cat(paste0(paste0(rep(" ", 5), collapse=""), 
#                    str_pad("FREX", longest_frex_word, side="right"), "PROB\n"), file=topic_file)
#         
#         for(word in 1:150) {
#             cat(paste0(str_pad(word, 3), ". ", str_pad(tmp$frex[topic, word], longest_frex_word, side="right"), 
#                        tmp$prob[topic, word], "\n"), file=topic_file, append=TRUE)
#         }
#     }
    rm(m)        
