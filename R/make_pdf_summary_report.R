...
make_pdf_summary_report <- function(stm_input_file, stm_results_dir, report_title,
    ...
    meta[ , era := cut(year, breaks=c(1976, 1990, 1995, 2000, 2005, 2010, 2015), 
    ...
    if("report" %in% names(full_stm_input$meta)) {
        ...
        num_reports <- length(unique(str_extract(full_report_meta$title, report_title_pattern)))
        full_report_meta[ , report_title := str_extract(full_report_meta$title, report_title_pattern)]
        full_report_meta <- full_report_meta[ , .(year=year[1], 
                                                  geog=geog[1]), 
    ...
    by_era <- eval(parse(text=sprintf("topics_by_docs[ , .(%s), by=era]", paste0(topic_labels, "=mean(", topic_labels, ")", collapse=", "))))
    ...
    by_era_and_geog <- eval(parse(text=sprintf("topics_by_docs[ , .(%s), by=.(era, geog)]", paste0(topic_labels, "=mean(", topic_labels, ")", collapse=", "))))
    
    
    all_geogs <- c("All Reports", geogs)
    
    
    era_and_geog_tab <- matrix(0, nrow=length(all_geogs), ncol=length(eras), dimnames=list(all_geogs, eras))
    ...
    # Top topic weights across time period, EU
    # eu_wts <- matrix(0, nrow=n_topics, ncol=length(eras), dimnames=list(as.character(1:n_topics), 
    #                                                                     eras))
    # 
    # eu_wts[ , "All Time Periods"] <- eu_topic_wts
    # 
    # for(t in eras) {
    #     if(t=="All Time Periods") next
    #     tmp <- by_era_and_geog[era==t & geog=="EU", topic_labels, with=FALSE]
    #     eu_wts[ , t] <- if(nrow(tmp)==1) {as.numeric(tmp)} else {NA}
    # }
    # 
    # setorder(by_era_and_geog, geog, era)
    
    # Top topic weights across time period, UN
    # un_wts <- matrix(0, nrow=n_topics, ncol=length(eras), dimnames=list(as.character(1:n_topics), 
    #                                                                     eras))
    # 
    # un_wts[ , "All Time Periods"] <- un_topic_wts
    # 
    # for(t in eras) {
    #     if(t=="All Time Periods") next
    #     tmp <- by_era_and_geog[era==t & geog=="UN", topic_labels, with=FALSE]
    #     un_wts[ , t] <- if(nrow(tmp)==1) {as.numeric(tmp)} else {NA}
    # }
    # 
    # setorder(by_era_and_geog, geog, era)
    
    # Top topic weights across time period, US
    # us_wts <- matrix(0, nrow=n_topics, ncol=length(eras), dimnames=list(as.character(1:n_topics), 
    #                                                                     eras))
    # 
    # us_wts[ , "All Time Periods"] <- us_topic_wts
    # 
    # for(t in eras) {
    #     if(t=="All Time Periods") next
    #     tmp <- by_era_and_geog[era==t & geog=="US", topic_labels, with=FALSE]
    #     us_wts[ , t] <- if(nrow(tmp)==1) {as.numeric(tmp)} else {NA}
    # }
    ###########################################################################
    ...
    # all_wts <- matrix(0, nrow=n_topics, ncol=3, dimnames=list(1:n_topics, c("EU", "UN", "US")))
    # all_wts[ , "EU"] <- eu_wts[ , 1]
    # all_wts[ , "UN"] <- un_wts[ , 1]
    # all_wts[ , "US"] <- us_wts[ , 1]
    ###########################################################################
    ...
    all_geogs_mtrx <- matrix(0, nrow=n_topics, ncol=length(eras), dimnames=list(as.character(1:n_topics), 
                                                                           eras))
